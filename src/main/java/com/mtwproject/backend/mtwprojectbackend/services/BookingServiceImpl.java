...
public class BookingServiceImpl implements BookingService {
    ...
    // Si la reserva es nueva (idBooking es nulo), establecer el estado a RESERVE_STATUS
    if (booking.getIdBooking() == null) {
        booking.setStatus(RESERVE_STATUS);
    } else {
        // Si la reserva ya existe, comprobar si idDriver no es nulo y el estado actual es RESERVE_STATUS
        if (booking.getDriver() != null && booking.getDriver().getIdDriver() != null && RESERVE_STATUS.equals(booking.getStatus())) {
            // Si idDriver no es nulo y el estado actual es RESERVE_STATUS, establecer el estado a PENDING_DRIVER_ASSIGNED_STATUS
            booking.setStatus(PENDING_DRIVER_ASSIGNED_STATUS);
        }
        // Si idDriver es nulo o el estado actual no es RESERVE_STATUS, no cambiar el estado
    }
